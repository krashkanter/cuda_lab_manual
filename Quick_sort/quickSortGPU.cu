...
// Iterative GPU QuickSort without CUDA Dynamic Parallelism
__global__ void gpu_quickSort(int *arr, int left, int right)
{
    // Manually allocated stack to avoid deep recursion limits
    int stack[128];
    int top = -1;
    ...
    while (top >= 0)
    {
        ...
        // Push left partition bounds to stack
        if (l < j)
        {
            stack[++top] = l;
            stack[++top] = j;
        }
        // Push right partition bounds to stack
        if (i < r)
        {
            stack[++top] = i;
            stack[++top] = r;
...
int main(int argc, char **argv)
{
    ...
    iStart = seconds();
    // Launching a single thread block to handle the iterative sort
    gpu_quickSort<<<1, 1>>>(d_A, 0, size - 1);
